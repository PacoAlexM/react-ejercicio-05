    ...
    return (
        <Card className="group overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-1 bg-linear-to-br from-white to-gray-50">
            <div className="relative h-64">
                <img onClick={ handleClick } src={ hero.image } alt={ hero.alias } className="object-cover transition-all duration-500 group-hover:scale-110 absolute top-[-30px] w-full h-[410px] cursor-pointer" />

                {/* Status indicator */}
                <div className="absolute top-3 left-3 flex items-center gap-2">
                    <div className={ `w-3 h-3 rounded-full ${hero.status === 'Active' ? 'bg-green-500' : 'bg-red-500'}` } />
                    <Badge variant="secondary" className="text-xs bg-white/90 text-gray-700">{ hero.status === 'Active' ? 'Activo' : 'No Activo' }</Badge>
                </div>
            ...
            <CardHeader className="py-3 z-10 bg-gray-100/50 backdrop-blur-sm relative top-1 group-hover:top-[-10px] transition-all duration-300">
                ...
                <Badge variant="outline" className="w-fit text-xs">{ hero.team }</Badge>
            </CardHeader>
                    ...
                    <div className="flex flex-wrap gap-1">
                        ...
                        }
                        {
                            hero.powers.length > 3 && (
                                <Badge variant="outline" className="text-xs bg-gray-100">+{ hero.powers.length - 3 } mas</Badge>
                            ) 
                        }
                    </div>
                </div>
